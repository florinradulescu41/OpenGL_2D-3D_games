...
{
	// First condition added so the cube does not move if we want to move the camera
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		if (window->KeyHold(GLFW_KEY_W)) translateZ -= deltaTime;
		if (window->KeyHold(GLFW_KEY_A)) translateX -= deltaTime;
		if (window->KeyHold(GLFW_KEY_S)) translateZ += deltaTime;
		if (window->KeyHold(GLFW_KEY_D)) translateX += deltaTime;
		if (window->KeyHold(GLFW_KEY_R)) translateY += deltaTime;
		if (window->KeyHold(GLFW_KEY_F)) translateY -= deltaTime;
	}
