...
void Laborator3_Vis2D::Init()
{
	...
	Mesh* square1 = Object2D::CreateSquare("square1", corner, length);
...
void Laborator3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->KeyHold(GLFW_KEY_W)) logicSpace.y += deltaTime; // Increase the vertical coordinate -> Up on the Y axis
	if (window->KeyHold(GLFW_KEY_A)) logicSpace.x -= deltaTime;	// Decrease the horizontal coordinate -> Left on the X axis
	if (window->KeyHold(GLFW_KEY_S)) logicSpace.y -= deltaTime;	// Decrease the vertical coordinate -> Down on the Y axis
	if (window->KeyHold(GLFW_KEY_D)) logicSpace.x += deltaTime;	// Increase the horizontal coordinate -> Right on the X axis

	//TODO zoom in and zoom out logic window with Z and X
	if (window->KeyHold(GLFW_KEY_Z)) {

		// Set the logic coordinates at the center of the logic window
		logicSpace.x += logicSpace.width / 2;
		logicSpace.y += logicSpace.height / 2;

		// Decrease the logic view -> A more narrow window -> Zoom in
		logicSpace.width -= deltaTime;
		logicSpace.height -= deltaTime;

		// Move back the coordinates for the new logic window
		logicSpace.x -= logicSpace.width / 2;
		logicSpace.y -= logicSpace.height / 2;
	}
	...
	if (window->KeyHold(GLFW_KEY_X)) {
		...
		// Increase the logic view -> A wider window -> Zoom out
		logicSpace.width += deltaTime;
		logicSpace.height += deltaTime;
