...
void Laborator8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	GLint angle = glGetUniformLocation(shader->program, "cut_off_angle");
	glUniform1f(angle, cutOffAngle); // cutoffAngle defined as 30
...
void Laborator8::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		// Move the spotlight direction and angle
		...
		if (typeOfLight == 1) {
			...
			if (window->KeyHold(GLFW_KEY_Z)) cutOffAngle += deltaTime * 100;
			if (window->KeyHold(GLFW_KEY_X)) cutOffAngle -= deltaTime * 100;

			glm::mat4 directionMatrix = glm::mat4(1);

			directionMatrix = glm::rotate(directionMatrix, angleOY, glm::vec3(0, 1, 0));
			directionMatrix = glm::rotate(directionMatrix, angleOX, glm::vec3(1, 0, 0));

			lightDirection = glm::vec3(0, -1, 0);	// Original value
			lightDirection = glm::vec3(directionMatrix * glm::vec4(lightDirection, 0));
...
{
	// add key press event
	if (key == GLFW_KEY_F) typeOfLight ^= 1;
